...
app.post('/api/expenses/sync', async (req, res) => {
    ...
    console.log(`Processing ${messages.length} SMS messages for user ${req.dbUser.id}`);
    ...
    const parseSms = (body) => {
        ...
        if (body.toLowerCase().includes('debited') || body.toLowerCase().includes('spent')) {
        ...
        return { amount, description: description.substring(0, 50), date: new Date() };
    };
