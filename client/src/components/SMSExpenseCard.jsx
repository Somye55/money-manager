...
const SMSExpenseCard = ({ expense, onImport, onDismiss, saving }) => {
  ...
  return (
    <div
      className="card p-3 mb-3 border-l-4"
    ...
    >
      ...
      <div className="flex justify-between items-center mb-3">
        <div>
          ...
          <p className="text-sm text-tertiary truncate max-w-[200px]">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 py-2 px-3 rounded-lg border border-border text-sm font-medium hover:bg-bg-secondary transition flex items-center justify-center gap-1 min-h-[44px]"
          disabled={saving}
          aria-label={`Dismiss expense from ${
            expense.merchant || "Unknown Merchant"
          }`}
        >
          <X size={16} className="text-tertiary" aria-hidden="true" />
          Dismiss
        </button>
        <button
          onClick={() => onImport(expense)}
          className="flex-1 py-2 px-3 rounded-lg bg-primary text-white text-sm font-medium hover:bg-primary-dark transition shadow-sm flex items-center justify-center gap-1 min-h-[44px]"
          disabled={saving}
          aria-label={`Add expense from ${
            expense.merchant || "Unknown Merchant"
          } for ${currencySymbol}${expense.amount.toFixed(2)}`}
        >
